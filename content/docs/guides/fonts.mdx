...
## Measure Generated Text

Use generated font metrics when layout code needs text width before rendering.
...
const metrics = createFontMetrics(generatedFonts['Roboto-Regular'], 32);
const width = measureFontText('Continue Watching', metrics, {
  letterSpacing: 1,
});
```

The measured width uses the generated glyph advances and kerning data for the
selected font.
